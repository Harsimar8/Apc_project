...
<<<<<<< HEAD
    ...
    public ResponseEntity<?> getIssuedBooks(@AuthenticationPrincipal UserDetails userDetails) {
        try {
            ...
            // Add issuedTo and dueDate if needed (mock or real)
            List<Map<String, Object>> booksWithExtra = new ArrayList<>();
            for (Book book : issuedBooks) {
                ...
                map.put("dueDate", book.getDueDate()); // make sure this field exists in Book entity
                booksWithExtra.add(map);
            }

            return ResponseEntity.ok(booksWithExtra);
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(Map.of("error", e.getMessage()));
        }
    }
